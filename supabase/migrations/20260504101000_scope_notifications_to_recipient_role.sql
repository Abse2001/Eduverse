alter table public.notifications
  add column if not exists recipient_role public.app_role;

update public.notifications
set recipient_role = case
  when type = 'assignment_submitted' then 'teacher'::public.app_role
...
drop index if exists idx_notifications_recipient_event_key;
create unique index if not exists idx_notifications_recipient_role_event_key
  on public.notifications (recipient_user_id, recipient_role, event_key)
  where event_key is not null;

drop index if exists idx_notifications_recipient_created;
create index if not exists idx_notifications_recipient_created
  on public.notifications (recipient_user_id, recipient_role, created_at desc);
...
create policy "users can read own role notifications"
...
create policy "users can mark own role notifications read"
  on public.notifications
  for update
  using (
    recipient_user_id = auth.uid()
...
drop function if exists public.create_person_notification(
  uuid,
  uuid,
  uuid,
  uuid,
...
begin
  ...
  if notification_type = 'assignment_submitted' then
    if target_recipient_role <> 'teacher' then
      raise exception 'Assignment submission notifications must target the teacher role';
    end if;
    ...
    if target_recipient_role <> 'student' then
      raise exception 'Assignment grade notifications must target the student role';
    end if;
